...
export type HeadlessPropGetter<P extends object = {}> = (
  props?: P,
...
export function useGrid<TItem>(options: UseGridOptions<TItem>): UseGridReturn<TItem> {
  throw new Error('media-ui-react headless API design only');
}

export function useLightbox<TItem>(
  options: LightboxOptions<TItem>,
): UseLightboxReturn<TItem> {
  throw new Error('media-ui-react headless API design only');
}

export function useReelSwiper<TItem>(
  options: ReelSwiperOptions<TItem>,
): UseReelSwiperReturn<TItem> {
  throw new Error('media-ui-react headless API design only');
}
